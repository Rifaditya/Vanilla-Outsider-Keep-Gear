...
Конфигурация сохраняется в формате JSON в файле `.minecraft/config/vanilla-outsider-keep-gear.json`. Поддерживает горячую перезагрузку без перезапуска сервера.

---

## 📋 🎒 Категории сохраняемых предметов
...
- [[💔 Прочность и штрафы за смерть|ru_ru-Durability-and-Penalties]]
